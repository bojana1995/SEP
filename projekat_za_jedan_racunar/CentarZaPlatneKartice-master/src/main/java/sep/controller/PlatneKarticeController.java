...
public class PlatneKarticeController {
	...
	public URLDTO usmeriNaBanku(@RequestBody CardDTO card) {
		...
		try {
			...
			cardAuthentificated = client.postForObject("http://localhost:8762/banka-service/payment/autentifikacijaUsera", entity,
					CardDTO.class);
		} catch (Exception e) {
		...
		if (cardAuthentificated != null) {
			...
			try {
				...
				url = client.postForObject("http://localhost:8762/banka-service/banka/izvrsiPlacanje", entity, URLDTO.class);

				logger.info("\n\t\tPreusmeravanje na URL (uspešno):\n" + url + "\n");
				return url;
			} catch (Exception e) {
				URLDTO url = new URLDTO();
				url.setUrl("http://localhost:8762/banka-service/greska.html");
